...
function App() {
  ...
  // ✅ 預設建議用 Gmail（若你要預設 mock，把 provider 改成 "mock"）
  const [envConfig, setEnvConfig] = useState({
    provider: "gmail", // "mock" | "gmail" | "smtp"
  ...
  const providerLabel =
    envConfig.provider === "mock"
      ? "模擬測試 (不寄信)"
      : envConfig.provider === "gmail"
      ? "Gmail"
      : "自訂 SMTP";
  ...
  return (
    <div className="container animate-fade-in">
      <header style={{ marginBottom: "2rem", textAlign: "center", position: "relative" }}>
        ...
        <p style={{ color: "var(--text-muted)" }}>行政作業自動化：匯入資料 ➔ 預覽確認 ➔ 寄送通知</p>
        ...
        {/* ✅ Debug 小區塊：確認目前前端拿到的 API_BASE（避免又回去 localhost） */}
        <div
          style={{
            marginTop: "1rem",
            display: "inline-flex",
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="help-section">
              ...
              <p style={{ color: "var(--text-muted)", fontSize: "0.9rem" }}>
                請確保您的 Excel 檔案包含以下對應欄位（系統支援常見欄位名稱）：
              </p>
            ...
            <div className="help-section">
              ...
              <p style={{ color: "var(--text-muted)", fontSize: "0.9rem" }}>
                因為安全性考量，寄信請使用「應用程式密碼」（不是登入密碼）：
              </p>
              <ol style={{ fontSize: "0.9rem", lineHeight: "1.6", paddingLeft: "1.5rem" }}>
                <li>登入 Google 帳號管理頁面（myaccount.google.com）。</li>
                <li>左側選「安全性」並開啟「兩步驟驗證」。</li>
                <li>搜尋「應用程式密碼」並進入。</li>
                <li>應用程式選「其他」→ 輸入名稱（如：學校系統）→ 產生。</li>
                <li>複製顯示的 16 位密碼，貼到本系統即可。</li>
              </ol>
            </div>
        ...
        <div>
            ...
            >
              <div>
                ...
                <p style={{ fontSize: "0.9rem", color: "var(--text-muted)", margin: 0 }}>
                  請確認每位教師的通知內容（<span style={{ color: "var(--primary)", fontWeight: "bold" }}>點擊該行</span>{" "}
                  可展開詳細信件預覽）。若 Email 顯示為「缺」，則無法寄送。
                </p>
              </div>
                  ...
                  {previewData.map((item, idx) => (
                    <React.Fragment key={idx}>
                      ...
                      >
                        ...
                        <td>
                          <span className="badge" style={{ background: "#e2e8f0" }}>
                            {item.data_rows.length} 筆
                          </span>
                        </td>
                        <td>{item.email ? <span style={{ color: "var(--success)" }}>已備妥</span> : <span style={{ color: "var(--text-muted)" }}>略過</span>}</td>
                      </tr>
                                    ...
                                    {item.data_rows.map((row, rIdx) => (
                                      <tr key={rIdx}>
                                        ...
                                        <td style={{ padding: "8px", border: "1px solid #cbd5e1" }}>
                                          {row.original_teacher} / {row.sub_teacher}
                                        </td>
                                      </tr>
                                    ))}
                                  </tbody>
          ...
          <div className="card" style={{ marginBottom: "1.5rem", border: "1px solid #e2e8f0" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1rem" }}>
              <div>
                <label style={{ display: "block", fontSize: "0.9rem", marginBottom: "0.3rem", fontWeight: 500 }}>服務供應商</label>
                <select
                ...
                <div style={{ marginTop: "0.4rem", fontSize: "0.85rem", color: "#64748b" }}>
                  目前模式：<b>{providerLabel}</b>
                </div>
              </div>

              <div>
                <label style={{ display: "block", fontSize: "0.9rem", marginBottom: "0.3rem", fontWeight: 500 }}>寄件者顯示名稱</label>
                <input
                  type="text"
                  className="input-field"
                  style={{ width: "100%", padding: "0.5rem", borderRadius: "4px", border: "1px solid #cbd5e1" }}
                  value={envConfig.sender_name}
                  onChange={(e) => setEnvConfig((prev) => ({ ...prev, sender_name: e.target.value }))}
                  placeholder="例如：教務處"
                />
              </div>
            </div>
            ...
            {envConfig.provider === "mock" ? (
              ...
              <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: "1rem" }}>
                <div>
                  <label style={{ display: "block", fontSize: "0.9rem", marginBottom: "0.3rem", fontWeight: 500 }}>SMTP 伺服器</label>
                  <input
                    type="text"
                    disabled={envConfig.provider === "gmail"}
                    className="input-field"
                    style={{
                      width: "100%",
                      padding: "0.5rem",
                      borderRadius: "4px",
                      border: "1px solid #cbd5e1",
                      background: envConfig.provider === "gmail" ? "#f1f5f9" : "white",
                    }}
                    value={envConfig.smtp_server}
                    onChange={(e) => setEnvConfig((prev) => ({ ...prev, smtp_server: e.target.value }))}
                  />
                </div>

                <div>
                  <label style={{ display: "block", fontSize: "0.9rem", marginBottom: "0.3rem", fontWeight: 500 }}>連接埠 (Port)</label>
                  <input
                    type="number"
                    className="input-field"
                    style={{ width: "100%", padding: "0.5rem", borderRadius: "4px", border: "1px solid #cbd5e1" }}
                    value={envConfig.smtp_port}
                    onChange={(e) => setEnvConfig((prev) => ({ ...prev, smtp_port: parseInt(e.target.value) || 0 }))}
                  />
                </div>
                ...
                <div style={{ alignSelf: "end" }}>
                  {envConfig.provider === "gmail" && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      style={{ fontSize: "0.85rem", color: "var(--primary)", textDecoration: "none", display: "block" }}
                    >
                      取得應用程式密碼 ↗
                    </a>
                  )}
                ...
                <div style={{ gridColumn: "1 / span 3", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                  <div>
                    <label style={{ display: "block", fontSize: "0.9rem", marginBottom: "0.3rem", fontWeight: 500 }}>寄件者 Email (帳號)</label>
                    <input
                      type="email"
                      className="input-field"
                      style={{ width: "100%", padding: "0.5rem", borderRadius: "4px", border: "1px solid #cbd5e1" }}
                      value={envConfig.smtp_user}
                      onChange={(e) => setEnvConfig((prev) => ({ ...prev, smtp_user: e.target.value }))}
                    />
                    {envConfig.provider === "gmail" && (
                      <div style={{ marginTop: "0.35rem", fontSize: "0.85rem", color: "#64748b" }}>
                        Gmail 寄信請填完整帳號（例如：teacher@school.edu.tw）
                      </div>
                    )}
                  </div>
                  ...
                  <div>
                    <label style={{ display: "block", fontSize: "0.9rem", marginBottom: "0.3rem", fontWeight: 500 }}>
                      {envConfig.provider === "gmail" ? "應用程式密碼（16 位）" : "密碼"}
                    </label>
                    <input
                      type="password"
                      className="input-field"
                      ...
                      onChange={(e) => setEnvConfig((prev) => ({ ...prev, smtp_password: e.target.value }))}
                      placeholder={envConfig.provider === "gmail" ? "請貼上 16 位應用程式密碼" : ""}
                    />
                    {envConfig.provider === "gmail" && (
                      <div style={{ marginTop: "0.35rem", fontSize: "0.85rem", color: "#b45309" }}>
                        小提醒：這裡要填「應用程式密碼」，不是 Gmail 登入密碼。
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: "1rem" }}>
            ...
            <button className="btn btn-primary" onClick={handleSend} disabled={loading || totalEmails - missingEmails === 0}>
      ...
      {step === 3 && (
        <div className="card animate-fade-in" style={{ textAlign: "center", padding: "3rem" }}>
          ...
          <h2>寄送完成</h2>
          <p style={{ color: "var(--text-muted)", marginBottom: "2rem" }}>作業已結束，以下是詳細執行報告。</p>
          ...
          <div style={{ textAlign: "left", maxHeight: "300px", overflowY: "auto", background: "#f8fafc", padding: "1rem", borderRadius: "8px", marginBottom: "2rem", border: "1px solid #e2e8f0" }}>
            {results.map((res, i) => (
              <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: "0.9rem", marginBottom: "0.5rem", paddingBottom: "0.5rem", borderBottom: "1px solid #eee" }}>
                ...
                <span style={{ color: res.status === "success" ? "#10b981" : res.status === "failed" ? "#ef4444" : "#94a3b8", fontSize: "0.85rem" }}>
